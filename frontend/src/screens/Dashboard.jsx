...
export default function Dashboard({ currentUser = null }) {
  ...
  const [mgrCreateError, setMgrCreateError] = useState(null);
      ...
      try {
        ...
        const normalized = arr.map((x) =>
          typeof x === "string"
            ? { id: x, name: x }
            : {
                id: x.id ?? x.branches ?? x.name,
                name: x.name ?? x.branches,
              }
        );
        if (!mounted) return;
        ...
        if (!selectedBranch && normalized.length) {
          if (
            (user?.role || "").toString().toLowerCase() === "manager" &&
            user?.branchId
          ) {
            setSelectedBranch(user.branchId);
          } else {
            setSelectedBranch(normalized[0].id);
          }
        }
      } catch (e) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const ovP = api
          .getBranchOverview(selectedBranch)
          .catch(() => null);
        const metricsParam = selectedFeatures.join(",");
        const tsP = api
          .getBranchTimeSeries(selectedBranch, { metrics: metricsParam })
          .catch(() => null);
        ...
        if (mounted) {
          setLoading((s) => ({ ...s, overview: false, series: false }));
        }
      }
    })();
    return () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        console.warn("regional error", e);
        setRegional([]);
      } finally {
        if (mounted) {
          setLoading((s) => ({ ...s, regional: false }));
        }
      }
    })();
    return () => {
  ...
  const { effectiveSeries, usingSynthetic } = useMemo(() => {
    function isSufficient(s) {
      return (
        s &&
        Array.isArray(s.labels) &&
        s.labels.length >= 2 &&
        Array.isArray(s.datasets) &&
        s.datasets.length > 0
      );
    }
    if (isSufficient(series)) {
      const filtered = {
        ...
        datasets: series.datasets.filter((ds) =>
          selectedFeatures.includes(ds.label)
        ),
      };
      const ds = filtered.datasets.length
        ? filtered.datasets
        : series.datasets;
      return {
        effectiveSeries: { labels: series.labels, datasets: ds },
    ...
    if (kpis && kpis.year && kpis.quarter) {
      startYear = Number(kpis.year);
      startQ = Number(kpis.quarter);
    } else {
      const now = new Date();
      startYear = now.getFullYear();
      const month = now.getMonth();
      startQ = Math.floor(month / 3) + 1;
    }
    ...
    const datasets = selectedFeatures.map((feat, idx) => {
      ...
      if (seed == null) {
        if (
          feat === "totalsavingsdeposits" &&
          kpis?.totalaggregatedeposits
        )
          seed = kpis.totalaggregatedeposits * 0.35;
        if (
          feat === "totalcurrentdeposits" &&
          kpis?.totalaggregatedeposits
        )
          seed = kpis.totalaggregatedeposits * 0.45;
        if (
          feat === "totaltermdeposits" &&
          kpis?.totalaggregatedeposits
        )
          seed = kpis.totalaggregatedeposits * 0.2;
        if (feat === "totalaggregatecredit" && kpis?.totalaggregatecredit)
          seed = kpis.totalaggregatecredit;
      }
      if (seed == null || Number.isNaN(seed)) {
        seed = Math.round(Math.random() * 3000 + 200);
      }
  ...
  const creditComponentBar = useMemo(() => {
    if (effectiveSeries && effectiveSeries.labels && effectiveSeries.datasets) {
      const findData = (label) =>
        effectiveSeries.datasets.find((d) => d.label === label)?.data ?? [];
      ...
      const any = [dsRetail, dsAgri, dsBiz].some(
        (d) => Array.isArray(d) && d.length && d.some((v) => v && v !== 0)
      );
      if (any) {
        return {
          labels: effectiveSeries.labels,
          datasets: [
            {
              label: "Retail",
              data: dsRetail,
    ...
    if (kpis?.totalaggregatecredit) {
      ...
      const labels =
        effectiveSeries?.labels && effectiveSeries.labels.length
          ? effectiveSeries.labels
          : ["current"];
      const periods = labels.length;
      const makeArray = (v) =>
        Array.from({ length: periods }, () => Math.round(v / periods));
      return {
        labels,
        datasets: [
          {
            label: "Retail",
            data: makeArray(retail),
            backgroundColor: "#06b6d4",
          },
          {
            label: "Agriculture",
  ...
  const topBranchesList = useMemo(() => {
    if (
      kpis?.topBranches &&
      Array.isArray(kpis.topBranches) &&
      kpis.topBranches.length
    ) {
      return kpis.topBranches.map((b, i) => ({
        id: b.branch_id ?? b.id ?? `b${i}`,
        name: b.name ?? b.branch_id ?? `Branch ${i + 1}`,
        deposits: Number(
          b.deposits ?? b.totalaggregatedeposits ?? 0
        ),
      }));
    }
  ...
  const monthlyTransactionBarData = useMemo(() => {
    ...
    if (s && s.labels && s.datasets) {
      const txnDs = s.datasets.find(
        (d) => d.label === "totaltransactionscount"
      );
      if (txnDs && Array.isArray(txnDs.data)) {
        return {
          labels: s.labels,
          datasets: [
            {
    ...
    if (
      kpis?.monthlyTransactions &&
      Array.isArray(kpis.monthlyTransactions)
    ) {
      return {
        labels: kpis.monthlyTransactions.map((m) => m.month),
        datasets: [
          {
            label: "Transactions",
            data: kpis.monthlyTransactions.map((m) => m.count),
            backgroundColor: "#4f46e5",
          },
        ],
      };
    }
    if (kpis?.totaltransactionscount) {
      return {
        labels: ["current"],
        datasets: [
          {
            label: "Transactions",
            data: [Number(kpis.totaltransactionscount)],
            backgroundColor: "#4f46e5",
          },
        ],
      };
  ...
  async function handleRegionClick(regionName) {
    ...
    try {
      const res = await api.getBranchesByRegion(regionName);
      setModalBranches(res?.data || []);
    } catch (e) {
      console.warn("branches-by-region error", e);
      setModalBranches([]);
    }
  ...
  async function exportBranchCSV() {
    try {
      const res =
        (api.getBranchGrid &&
          (await api.getBranchGrid(selectedBranch))) ||
        (await api.getBranchOverview(selectedBranch));
      const data = res.data;
      const keys =
        Array.isArray(data) && data.length
          ? Object.keys(data[0])
          : Object.keys(data || {});
      const csvRows = Array.isArray(data)
        ? [keys.join(",")].concat(
            data.map((r) =>
      ...
      a.href = url;
      a.download = `${selectedBranch || "branch"}-export.csv`;
      a.click();
  ...
  const depositMixDonut = useMemo(() => {
    ...
    if (effectiveSeries && effectiveSeries.labels && effectiveSeries.datasets) {
      ...
      const findLast = (label) => {
        const ds = effectiveSeries.datasets.find(
          (d) => d.label === label
        );
        if (ds && Array.isArray(ds.data)) {
          return Number(ds.data[lastIdx] ?? 0);
        }
        return 0;
      };
      const cur = findLast("totalcurrentdeposits");
      const sav = findLast("totalsavingsdeposits");
      const term = findLast("totaltermdeposits");
      if (cur || sav || term) {
        return {
          labels: ["Current", "Savings", "Term"],
          datasets: [
    ...
    const curVal = Math.round(tot * 0.45) || 0;
    const savVal = Math.round(tot * 0.35) || 0;
    const termVal = Math.round(tot * 0.2) || 0;
    return {
      labels: ["Current", "Savings", "Term"],
      datasets: [
        {
          data: [curVal, savVal, termVal],
          backgroundColor: ["#7c3aed", "#06b6d4", "#f59e0b"],
        },
      ],
    };
  ...
  async function handleCreateManager() {
    setMgrCreateError(null);

    const effectiveBranchId =
      mgrForm.branchId || mgrForm.branchIdManual?.trim();

    if (
      !mgrForm.name.trim() ||
      !mgrForm.email.trim() ||
      !mgrForm.username.trim() ||
      !effectiveBranchId
    ) {
      setMgrCreateError(
        "Please fill name, email, username and assign a branch (dropdown or manual)."
      );
      return;
    }
    ...
    // send both naming styles so it works with either backend version
    const payload = {
    ...
    try {
      ...
      const res = api.createManager
        ? await api.createManager(payload)
        : { data: { ok: true } };
      console.log("createManager response:", res?.data);
      setMgrCreatedInfo({
      ...
      // clear form (keep created info visible)
      setMgrForm({
        name: "",
        email: "",
        username: "",
    ...
    >
      ...
      <main style={{ flex: 1, padding: "24px 32px", maxWidth: "100%" }}>
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <img
              ...
              onError={(e) => {
                ...
                e.currentTarget.src = "/images/fallback-bank.png";
              }}
            />
            <div>
          ...
          >
            ...
            {!isManager ? (
              ...
              >
                <option
                  value={user?.branchId ?? selectedBranch ?? ""}
                >
                  {branches.find(
                    (b) =>
                      b.id ===
        ...
        >
          ...
          >
            <div style={{ fontSize: 12, color: PALETTE.neutral }}>
              Total Deposits
            </div>
            <div style={{ fontSize: 20, fontWeight: 800 }}>
              {formatNumber(kpis?.totalaggregatedeposits)}
            </div>
            <div style={{ marginTop: 8 }}>
              <SparkSmall data={effectiveSeries ?? {}} />
            </div>
          </div>
          ...
          >
            ...
            <div style={{ fontSize: 20, fontWeight: 800 }}>
              {formatNumber(kpis?.totalaggregatecredit)}
            </div>
          </div>
          ...
          >
            ...
            <div style={{ fontSize: 20, fontWeight: 800 }}>
              {formatNumber(kpis?.totaltransactionscount)}
            </div>
          </div>
        </section>
        ...
        {usingSynthetic && (
          ...
          >
            Showing synthetic quarterly trend curves based on the
            latest KPIs for this branch.
          </div>
        )}
        ...
        >
          ...
          >
            ...
            <div style={{ height: 360 }}>
              {effectiveSeries &&
              effectiveSeries.labels &&
              effectiveSeries.labels.length >= 1 ? (
                <StackedAreaChart data={effectiveSeries} />
              ) : (
                <div
                  style={{
                    height: 320,
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ height: 220, overflow: "hidden" }}>
                  {creditComponentBar ? (
                    ...
                    <div style={{ color: PALETTE.neutral }}>
                      No credit split data available for this
                      branch/period.
                    </div>
                  )}
                </div>
          ...
          >
            ...
            {/* Monthly Transaction - small bar chart */}
            <div
              style={{
                background: PALETTE.card,
                padding: 16,
        ...
        {managerModalOpen && (
          ...
          >
            ...
            >
              ...
              >
                You can either pick a branch from the dropdown or paste
                a branch id manually. If both are filled, the dropdown
                value is used.
              </div>
              ...
              {mgrCreatedInfo && (
                ...
                >
                  ...
                  <div>
                    Username:{" "}
                    <code>{mgrCreatedInfo.username}</code>
                  </div>
                  <div>
                    Temporary password:{" "}
                    <code>{mgrCreatedInfo.password}</code>
                  </div>
                  <div
                    style={{
                      fontSize: 12,
                  ...
                  >
                    Share these credentials with the manager. They will
                    be forced to change the password at first login.
                  </div>
                </div>
              )}
